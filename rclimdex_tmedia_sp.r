...
cbvalue1<-tclVar("0");cbvalue2<-tclVar("0");cbvalue3<-tclVar("0")
cbvalue4<-tclVar("0");cbvalue5<-tclVar("0");cbvalue6<-tclVar("0")
cbvalue7<-tclVar("1");cbvalue8<-tclVar("0");cbvalue9<-tclVar("0")
cbvalue10<-tclVar("0");cbvalue11<-tclVar("0");cbvalue12<-tclVar("0")
cbvalue13<-tclVar("0");cbvalue14<-tclVar("0");cbvalue15<-tclVar("0")
cbvalue16<-tclVar("0");cbvalue17<-tclVar("0");cbvalue18<-tclVar("0")
cbvalue19<-tclVar("0");cbvalue21<-tclVar("0")
...
dtr<-function(){# day temperature range(monthly average) 
  ...
  dimnames(aa1)<-list(NULL,c("year","month","tmean"))
  aa1[,"year"]<-years:yeare
  aa1[,"year"]<-mysort(aa1[,"year"],decreasing=F)
  aa1[,"month"]<-1:12
  temrange<-(dd[,"tmax"]+dd[,"tmin"])/2
  ...
  for (year in years:yeare){    # start year loop
    ...
    aa1[,"tmean"]<-aa1[,"tmean"]+nacor[,"mnatma>3"]+nacor[,"mnatmi>3"]
  ...
  cat(file=namt,paste(latitude,longitude,"tmean",years,yeare,betahat,betastd,pvalue,sep=","),fill=180,append=T)

    nam2<-paste(outjpgdir,paste(ofilename,"_TMEAN.png",sep=""),sep="/")
    png(nam2,width=1024,height=768,bg="white")
    plotx(ofile[,1],ofile[,14],main=paste("TMEAN",ofilename,sep="   "),xlab="Año",ylab="T media (°C)")
    dev.off()
    nam2a<-paste(outjpgdir,paste(ofilename,"_TMEAN.pdf",sep=""),sep="/")
    pdf(nam2a)
    plotx(ofile[,1],ofile[,14],main=paste("TMEAN",ofilename,sep="   "),xlab="Año",ylab="T media (°C)")
    dev.off()
   } # end of dtr
